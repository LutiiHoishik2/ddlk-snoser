...
class RabanokBot:
    ...
    def register_handlers(self):
        ...
        self.dp.message.register(
            ...
            StateFilter(UserStates.waiting_for_target)
        )
        self.dp.message.register(         
            self.user_handlers.process_deposit_amount,
            StateFilter(UserStates.waiting_deposit_amount)
        )
        
        # === ДОБАВЛЕННОЕ СОСТОЯНИЕ ДЛЯ ПРИЧИНЫ ЖАЛОБЫ ===
        self.dp.message.register(
            self.user_handlers.process_reason_input,
            StateFilter(UserStates.waiting_for_reason)
        )
        ...
        self.dp.message.register(
            ...
            StateFilter(AdminStates.waiting_for_subscription_user)
        )

        # Управление сессиями
        self.dp.message.register(
            self.admin_handlers.process_session_phone,
            StateFilter(AdminStates.waiting_for_session_phone)
        )
        self.dp.message.register(
            self.admin_handlers.process_session_code,
            StateFilter(AdminStates.waiting_for_session_code)
        )
        self.dp.message.register(
            self.admin_handlers.process_session_password,
            StateFilter(AdminStates.waiting_for_session_password)
        )

        # Управление почтами
        self.dp.message.register(
            self.admin_handlers.process_emails_file,
            StateFilter(AdminStates.waiting_for_emails_file)
        )
        self.dp.message.register(
            self.admin_handlers.process_email_input,
            StateFilter(AdminStates.waiting_for_manual_emails)
        )
        self.dp.message.register(
            self.admin_handlers.process_email_password,
            StateFilter(AdminStates.waiting_for_email_password)
        )
        self.dp.message.register(
            self.admin_handlers.process_single_email,
            StateFilter(AdminStates.waiting_for_single_email)
        )
